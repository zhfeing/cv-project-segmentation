...
class Trainer(object):
    def __init__(self, args):
        ...
        train_sampler = distributed.make_data_sampler(
            train_dataset,
            shuffle=True,
            distributed=args.distributed
        )
        train_batch_sampler = distributed.make_batch_data_sampler(
            train_sampler,
            args.batch_size,
            args.max_iters
        )
        val_sampler = distributed.make_data_sampler(
            val_dataset,
            False,
            args.distributed
        )
        val_batch_sampler = distributed.make_batch_data_sampler(val_sampler, args.batch_size)
    ...
    def train(self):
        save_to_disk = distributed.get_rank() == 0
...
def save_checkpoint(model, args, is_best=False):
    """Save Checkpoint"""
    directory = os.path.abspath(args.save_dir)
...
if __name__ == "__main__":
    ...
    logger = setup_logger(
        ...
        filename='{}_{}_{}.log'.format(args.model, args.backbone, args.dataset)
    )
